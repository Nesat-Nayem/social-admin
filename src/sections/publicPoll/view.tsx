...
import { IPublicItem, IPublicTableFilters, IPublicTableFilterValue } from 'src/types/public-poll';

import PublicPollFiltersResult from './public-table-filters-result';
import {
  RenderCellNo,
  RenderCellYes,
  RenderCellTitle,
...
const defaultFilters: IPublicTableFilters = {
  services: [],
};
...
export default function PublicPollView() {
  ...
  const [tableData, setTableData] = useState<IPublicItem[]>([]);
  ...
  const handleFilters = useCallback((name: string, value: IPublicTableFilterValue) => {
  ...
  const handleViewRow = useCallback(
    (id: string) => {
      router.push(`${paths.dashboard.publicPoll.details}/${id}`);
    },
    [router]
  );
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      minWidth: 260,
      hideable: false,
      renderCell: (params) => <RenderCellTitle params={params} />,
    },
    {
      field: 'yes',
      headerName: 'Yes count',
      width: 160,
      renderCell: (params) => <RenderCellYes params={params} />,
    },

    {
      ...
      renderCell: (params) => <RenderCellNo params={params} />,
    },
    {
      field: 'publishedDate',
...
function applyFilter({
  ...
  inputData: IPublicItem[];
  filters: IPublicTableFilters;
}) {
